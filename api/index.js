...
function getProducts(request, response) {
  ...
  let data = []
  if (category_id > 0) {
    const sqlOpdracht = db.prepare('SELECT products.id, products.code, products.name, products.description, products.price, ratings.rating, MinimumAge.MinimumAge FROM products JOIN ratings ON ratings.id = products.rating_id JOIN MinimumAge ON MinimumAge.id = products.MinimumAge_id WHERE category_id = ? ORDER BY name ASC')
    data = sqlOpdracht.all(category_id)
  } else {
    const sqlOpdracht = db.prepare('SELECT products.id, products.code, products.name, products.description, products.price, ratings.rating, MinimumAge.MinimumAge FROM products JOIN ratings ON ratings.id = products.rating_id JOIN MinimumAge ON MinimumAge.id = products.MinimumAge_id ORDER BY name ASC')
